...
**Date:** January 2026  
**Hardware:** 
- Google Colab Pro (A100 80GB) — Diffusion model training & image generation
- Local PC (RTX 5070TI 16GB + 32GB RAM) — Classifier training & experiments
...
│  │                    COLAB PRO (A100 80GB)                            │   │
│  ├─────────────────────────────────────────────────────────────────────┤   │
│  │  1. Dataset Prep    2. Full DreamBooth       3. Generate Synthetic  │   │
│  │  ┌───────────┐      ┌─────────────────┐      ┌─────────────────┐   │   │
│  │  │ Chest X-ray│ ──► │ SD 2.1 + Text   │ ──►  │ 1000+ synthetic │   │   │
│  │  │ Dataset    │      │ Encoder Training│      │ images/class    │   │   │
│  │  └───────────┘      └─────────────────┘      └─────────────────┘   │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                    │                                        │
│                                    ▼ Download synthetic images              │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                 LOCAL PC (RTX 5070TI 16GB)                          │   │
│  ├─────────────────────────────────────────────────────────────────────┤   │
│  │  4. Quality Eval    5. Classifier Training    6. Results Analysis   │   │
│  │  ┌───────────┐      ┌─────────────────┐      ┌─────────────────┐   │   │
│  │  │ FID Score │      │ 7 Experiments   │      │ Statistical     │   │   │
│  │  │ MS-SSIM   │      │ DenseNet-121    │      │ Comparison      │   │   │
│  │  └───────────┘      └─────────────────┘      └─────────────────┘   │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
...
```
Forward:  Clean Image → Slightly Noisy → Very Noisy → Pure Noise
Reverse:  Pure Noise → Slightly Less Noisy → ... → Generated Image
```
...
|------|-------------------|---------------|
| Diffusion model finetuning | 🔴 Very High (40-80GB VRAM) | Colab A100 80GB |
| Synthetic image generation | 🟠 High (8-16GB VRAM) | Colab A100 80GB |
| FID/quality evaluation | 🟡 Medium (4-8GB VRAM) | Either |
| Classifier training | 🟢 Low (4-6GB VRAM) | Local RTX 5070TI |
| Experiments & analysis | 🟢 Low | Local PC |

### Benefits of This Split

1. **Save Colab compute units** — Classifier training is cheap, don't waste A100 time on it
2. **Faster iteration** — Run classifier experiments locally without internet dependency
3. **Parallel work** — Generate more synthetic images on Colab while training locally
4. **No timeouts** — Local training won't disconnect after 90 minutes of "inactivity"

### Your Hardware Capabilities

#### Colab Pro A100 80GB
```
✅ Full DreamBooth finetuning (not just LoRA)
✅ Train text encoder (better medical terminology)
✅ Batch size 8-16 for training
✅ Generate thousands of synthetic images
✅ Run quality metrics (FID) on large sets
```

#### Local PC (RTX 5070TI 16GB + 32GB RAM)
```
✅ DenseNet-121 classifier with batch size 64-128
✅ ResNet-50 classifier with batch size 32-64
✅ Vision Transformer (ViT) with batch size 16-32
✅ All 7 experiments in a few hours
...
|-------|-----------|-------------|---------------|----------------|
| **Stable Diffusion 1.5** | 512×512 | ~12GB | Fast | Good but older |
| **Stable Diffusion 2.1** | 512×512 | ~16-24GB | Medium | ⭐⭐ **Best choice** |
| **SDXL** | 1024×1024 | ~40GB+ | Slow | Overkill for CXR |
| **SD 3.0/3.5** | Various | ~50GB+ | Very Slow | Too new |

### 🏆 Recommended: Stable Diffusion 2.1 Base

**Why SD 2.1 for your project:**
- ✅ 512×512 resolution matches common CXR preprocessing
- ✅ Well within your 80GB VRAM budget
- ✅ Proven in medical imaging research (RoentGen, Medfusion)
- ✅ Can train text encoder for better medical terminology
- ✅ OpenRAIL++ license allows research use
...
1. **Resolution overkill**: Classifiers resize to 224×224 anyway
...
|--------|--------------|-------------|---------|
| **Full DreamBooth** | Updates all model weights | ~24-40GB | ⭐⭐⭐ Best |
| **DreamBooth + LoRA** | Low-rank adaptation | ~12-16GB | ⭐⭐ Great |
| **LoRA only** | Trains small adapter | ~8-12GB | ⭐ Good |

### 🏆 Recommended: Full DreamBooth with Text Encoder

**With your 80GB A100, you can use the highest quality approach!**

Full DreamBooth advantages:
- ✅ Best image quality and realism
- ✅ Better understanding of medical terminology (text encoder training)
- ✅ No quality compromise from low-rank approximation
...
|--------|------------------|------------|
| **FID** (Fréchet Inception Distance) | Distribution similarity | Lower is better (< 50 good, < 20 excellent) |
...
def run_all_experiments(
    ...
    for exp_name, config in experiments.items():
        ...
        print(f"\n{exp_name} Results: Acc={np.mean(exp_accuracies):.4f}±{np.std(exp_accuracies):.4f}")
...
def analyze_and_print_results(results: dict):
    ...
    for exp_name, data in results.items():
        acc_str = f"{data['accuracy_mean']:.4f} ± {data['accuracy_std']:.4f}"
        f1_str = f"{data['f1_mean']:.4f} ± {data['f1_std']:.4f}"
...
╠════════════════════╪═══════════════╪══════════╪════════╪════════════════╣
║ Baseline           │ 94.2% ± 0.3%  │  0.943   │ 0.967  │      -         ║
║ Real + 25% Synth   │ 94.8% ± 0.4%  │  0.948   │ 0.971  │  +0.6%         ║
║ Real + 50% Synth   │ 95.3% ± 0.3%  │  0.952   │ 0.974  │  +1.1%*        ║
║ Real + 100% Synth  │ 95.7% ± 0.2%  │  0.956   │ 0.978  │  +1.5%*        ║
║ 10% Real + Synth   │ 91.2% ± 0.5%  │  0.908   │ 0.945  │  +6.1%**†      ║
║ 25% Real + Synth   │ 93.8% ± 0.4%  │  0.936   │ 0.962  │  +2.4%*†       ║
║ Synthetic Only     │ 87.3% ± 0.6%  │  0.871   │ 0.912  │  -6.9%         ║
╚══════════════════════════════════════════════════════════════════════════╝
* p < 0.05 vs baseline | ** p < 0.01 vs baseline | † vs limited real baseline
```

### Key Graphs for Poster

1. **Real vs Synthetic Comparison** — Side-by-side image grid
2. **FID Score** — Bar chart (before/after finetuning)
3. **Accuracy by Configuration** — Bar chart with error bars
4. **Training Curves** — Loss over epochs
5. **ROC Curves** — Compare AUC across experiments
